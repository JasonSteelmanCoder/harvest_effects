...
for i in range(len(data)):
    ...
    final_obs_year = data[i]["plot_year_sequence"][-1]

    if str(data[i]["plot_original_cn"]) not in death_counts:
        death_counts[str(data[i]["plot_original_cn"])] = {"am deaths": 0, "em deaths": 0, "years after harvest": final_obs_year - harvest_year}
...
for value in death_counts.values():
    if value["years after harvest"] > 0:
        am_death_per_year_values.append(value["am deaths"] / value["years after harvest"])
        em_death_per_year_values.append(value["em deaths"] / value["years after harvest"])
