	...
	WITH filtered_plot_observations AS (
		WITH harvest_sequences AS (
			WITH harvested_plots AS (
				WITH conditioned_plot_years AS (
					WITH harvested_plot_years AS (
						...
						-- make an array of the stdorgcd's of all the conditions on the plot observation
						SELECT 
							mopy.*,
							MAX(CASE 
					...
					SELECT 
						...
						yr,
						harvested,
						condition_should_exclude_this_plot,
						CASE WHEN 1 = ANY(condition_should_exclude_this_plot) THEN 1 ELSE 0 END AS exclude_this_plot
					FROM harvested_plot_years hpy
				)
				-- group by plot again, turning cn yr, harvested, and plot_exclusion_flags into lists (this will allow us to filter out extraneous plots before searching the tree table)
				SELECT 
					original_cn,
					...
					ARRAY_AGG(exclude_this_plot ORDER BY yr) AS plot_exclusion_flags
				FROM conditioned_plot_years cpy
				GROUP BY original_cn
			)
			-- filter out plots with exclusion flags or whose harvest sequences don't work for interrupted time series analysis
			SELECT 
				*
			...
			WHERE harvested_sequence[1] = 0					-- the plot must not be harvested on the first observation
				...
				AND ARRAY_POSITION(plot_exclusion_flags, 1) IS NULL		-- the plot must not have any conditions that exclude it
